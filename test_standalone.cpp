...
int main() {
    ...
    {
        // Names are vetted by the section above; skip nulls rather than
        // crashing if a table regeneration drops one.
        tp::FeatureBits undetectable{};
        for (const char *const *p =
            ...
            if (const tp::FeatureEntry *fe = tp::find_feature(*p))
                tp::feature_set(&undetectable, fe->bit);
        ...
        if (ci_cpus[0] == nullptr) {
            ...
            for (const char *const *p = ci_cpus; *p; p++) {
                const tp::CPUEntry *cpu = tp::find_cpu(*p);
                if (!cpu) { printf("  %s: NOT IN TABLE (skip)\n", *p); continue; }
                tp::FeatureBits inter;
                tp::feature_and_out(&inter, &cpu->features, &undetectable);
                bool ok = !tp::feature_any(&inter);
                check(ok, (std::string("CI CPU '") + *p +
                           "' must not contain undetectable feature bits").c_str());
                if (!ok) {
                    any_fail = true;
                    for (unsigned i = 0; i < tp::num_features; i++)
                        if (tp::feature_test(&inter, tp::feature_table[i].bit))
                            printf("    undetectable in %s: %s\n",
                                   *p, tp::feature_table[i].name);
                }
            }
            printf("  %s\n", any_fail ? "FAILED" : "OK");
        }
